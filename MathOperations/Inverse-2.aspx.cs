...
{
    public partial class Inverse_2 : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            for (j = 0; j < 3; j++)
            {
                determinant = determinant + (m[0, j] * (m[1, (j + 1) % 3] * m[2, (j + 2) % 3] - m[1, (j + 2) % 3] * m[2, (j + 1) % 3]));
            }
            for (j = 0; j < 3; j++)
            {
                for (k = 0; k < 3; k++)
                {
                    i[j, k] = ((m[(j + 1) % 3,(k + 1) % 3] * m[(j + 2) % 3,(k + 2) % 3]) - (m[(j + 1) % 3,(k + 2) % 3] * m[(j + 2) % 3,(k + 1) % 3])) / determinant;
                }
            }
